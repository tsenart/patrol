...
func TestRate(t *testing.T) {
	r := Rate{Freq: 100, Per: time.Second}
	d := r.Duration(1000)
	if have, want := d, r.Duration(r.Tokens(d)); have != want {
		t.Errorf("rate: %+v: have: %d, want: %d", r, have, want)
	}
}
...
func TestBucket_Take(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{elapsed: time.Millisecond, take: 2, ok: false, rem: 0}, // not enough tokens
